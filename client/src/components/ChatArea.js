...
import { MessageSquare, MoreVertical, Phone, Video, ArrowLeft } from 'lucide-react';
...
export default function ChatArea({ chat, socket, onBack }) {
  const { user } = useAuth();
  ...
  if (!chat) {
    return (
      <div
        className="chat-area-panel mobile-hide-panel"
        style={{
          flex: 1,
          height: '100%',
